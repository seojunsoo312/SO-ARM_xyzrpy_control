...
기존 AABB(`datasets/raw/labels`)는 그대로 둔다.
결과는 `datasets/raw/labels_seg`. 빈 책상(박스 없음)은 빈 txt.

  python yolo/auto_seg.py
  python yolo/auto_seg.py --force          # 이미 있는 마스크도 다시
  python yolo/auto_seg.py --model sam_b.pt # 더 정확, VRAM 큼
"""
...
from yolo.boxes import load_yolo_txt, save_yolo_seg_txt, simplify_polygon_px
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(
            ...
            "conda activate lerobot 한 뒤: python yolo/auto_seg.py"
        )
    ...
    for img_path in images:
        ...
        if not boxes:
            ...
            continue
        xyxy = [[float(x1), float(y1), float(x2), float(y2)] for x1, y1, x2, y2 in boxes]
        try:
            results = sam.predict(
                bgr,
                bboxes=xyxy,
    ...
    print("다음: python yolo/review_seg.py")
