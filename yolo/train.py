#!/usr/bin/env python3
"""YOLO11n 을 이 보드에서 1-클래스 파인튜닝한다.

COCO 에 없는 물건이어도 pretrained nano 가중치에서 시작하는 편이 낫다.
클래스 의미는 data.yaml 의 이름 하나뿐이다.

  python yolo/train.py
  python yolo/train.py --seg
  python yolo/train.py --epochs 50 --batch 2
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="1-class YOLO11n train")
    ...
    start_model = args.model or ("yolo11n-seg.pt" if args.seg else "yolo11n.pt")
    run_name = "train_seg" if args.seg else "train"
    dest = BEST_SEG_PT if args.seg else BEST_PT

    train_dir = CUSTOM_ROOT / "images" / "train"
    n_train = len(list(train_dir.glob("*"))) if train_dir.exists() else 0
    if n_train < 8:
        raise SystemExit("데이터셋이 없습니다. python yolo/prepare.py 를 먼저 실행하세요.")
    ...
    print("다음: python yolo/detect.py --seg" if args.seg else "다음: python yolo/detect.py")
